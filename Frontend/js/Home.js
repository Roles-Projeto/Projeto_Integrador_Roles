document.addEventListener("DOMContentLoaded", () => {
    ...
    async function iniciarCarrossel() {
        ...
        function proximoCard()   { currentIndex = (currentIndex + 1) % total;            aplicarCarrossel(); }
        function cardAnterior()  { currentIndex = (currentIndex - 1 + total) % total;    aplicarCarrossel(); }
    ...
    if (catTrack && catCards.length > 0) {
        ...
        function maxIndex() { return Math.max(0, catCards.length - visiveis()); }
        ...
        function proximaCategoria()  { catIndex++; if (catIndex > maxIndex())  catIndex = 0;           moverCategorias(catIndex === 0); }
        function categoriaAnterior() { catIndex--; if (catIndex < 0)           catIndex = maxIndex();  moverCategorias(catIndex === maxIndex()); }
        function iniciarAutoplay()   { clearInterval(catAutoplay); catAutoplay = setInterval(proximaCategoria, 3000); }
        ...
        let touchStartX = 0;
        catTrack.addEventListener("touchstart", (e) => { touchStartX = e.touches[0].clientX; clearInterval(catAutoplay); }, { passive: true });
        catTrack.addEventListener("touchend",   (e) => {
            const diff = touchStartX - e.changedTouches[0].clientX;
            if (Math.abs(diff) > 40) diff > 0 ? proximaCategoria() : categoriaAnterior();
            iniciarAutoplay();
        }, { passive: true });

        window.addEventListener("resize", () => { catIndex = Math.min(catIndex, maxIndex()); moverCategorias(); });
