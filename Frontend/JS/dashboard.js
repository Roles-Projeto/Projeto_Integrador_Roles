/* ════════════════════════════════════════════════
   DASHBOARD.JS — ROLÊS  (versão funcional)

   O que funciona sem banco de dados:
   ✔ Badge de notificações dinâmico (atualiza ao ler)
   ✔ Alerta de topo dinâmico (some quando zera notificações)
   ✔ Gráficos recriados ao voltar para Visão Geral
   ✔ Botões "Criar Evento" / "Novo Estabelecimento" → redirect
   ✔ Botões "Editar" → modal de edição in-page
   ✔ Botões "Ingressos" → modal com input funcional
   ✔ Filtro de vendas (busca + status)
   ✔ Filtro de notificações por categoria
   ✔ Marcar todas como lidas (badge, alerta e itens atualizam)
   ✔ Exportar Relatório Excel
   ✔ Período dos gráficos (6M / 3M / 1M)
   ════════════════════════════════════════════════ */
...
function initNavButtons() {
  ...
  const PATHS = {
    criarEvento: '../criareventos/criareventos.html',
    criarEstabelecimento: '../criarEstabelicimentos/criarEstabelecimentos.html',
  };
