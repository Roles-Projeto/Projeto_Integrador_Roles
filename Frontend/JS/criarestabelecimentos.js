...
// VARIÁVEIS GLOBAIS
// ====================================================
const API_URL = "http://localhost:3000/estabelecimentos"; // ← BANCO DE DADOS
...
function salvarRascunho() {
    ...
    document.querySelectorAll("input:not([type='file']), textarea, select").forEach(input => {
        if (!input.id) return;
        dados[input.id] = input.type === "checkbox" || input.type === "radio"
            ? input.checked
            : input.value;
    });
...
function mostrarResumo() {
    ...
    if (listaPratos.length > 0) {
        ...
        listaPratos.forEach(p => {
            const valorDisplay = p.preco ? `R$ ${p.preco}` : "Sem preço";
            const badge        = p.tipo === "destaque" ? "⭐ Destaque" : "🏷️ Promoção";
            pratosHTML += `<li>${badge} — <strong>${p.titulo}</strong> (${p.categoria}) — ${valorDisplay}</li>`;
        });
        pratosHTML += "</ul>";
    }
...
// ====================================================
// CONFIRMAR PUBLICAÇÃO — ENVIA PARA O BANCO
// ====================================================
confirmarBtn.addEventListener("click", async () => {
    ...
    document.querySelectorAll(".horario-dia-row").forEach(row => {
        horarios.push({
            dia       : row.dataset.dia,
            aberto    : row.querySelector(".dia-check")?.checked || false,
            abertura  : row.querySelector(".hora-abertura")?.value  || "",
            fechamento: row.querySelector(".hora-fechamento")?.value || "",
        });
    });
    ...
    const estabelecimento = {
        nome          : document.getElementById("estab-name")?.value?.trim()  || "",
        tipo,
        especialidade : document.getElementById("especialidade")?.value        || "",
        faixa_preco   : document.getElementById("faixa-preco")?.value          || "",
        capacidade    : document.getElementById("capacidade")?.value            || "",
        descricao     : document.getElementById("descricao")?.value             || "",
        local_nome    : document.getElementById("local-nome")?.value            || "",
        ...
        responsavel   : document.getElementById("owner-name")?.value?.trim()   || "",
        ...
        horarios,
        pratos        : [...listaPratos],
        categoria_card: mapearCategoria(tipo),
    };
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(data.erro || "Erro ao publicar estabelecimento.");

        alert(`✅ "${estabelecimento.nome}" publicado com sucesso!`);
...
function criarFormPrato(tipo) {
    ...
    const camposValor = tipo === "promocao" ? `
    ...
    ticketItem.querySelector(".btnSalvarPrato").addEventListener("click", () => {
        ...
        if (tipo === "destaque") {
            ...
            preco = ticketItem.querySelector(".preco-promo")?.value || "";
        }
...
function renderizarPratos() {
    ...
    listaPratos.forEach((prato, index) => {
        ...
        item.innerHTML = `
            <div><strong>${prato.titulo}</strong> &nbsp;— ${prato.categoria} — ${valorDisplay} <em>(${badge})</em></div>
            <div>
...
function editarPrato(index) {
    ...
    const precoField = form.querySelector(".preco-destaque") || form.querySelector(".preco-promo");
