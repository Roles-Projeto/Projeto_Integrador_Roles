...
exports.cadastrarEmpresario = async (req, res) => {
  ...
  if (!nome_responsavel || !email || !nome_estabelecimento || !cnpj || !senha) {
    return res.status(400).json({
      erro: "Campos obrigatórios: nome_responsavel, email, nome_estabelecimento, cnpj e senha."
    });
  }
    ...
    connection.query(
      ...
      async (err, results) => {
        ...
        connection.query(
          ...
          (plano, nome_responsavel, email, nome_estabelecimento, cnpj,
          tipo_estabelecimento, telefone_comercial, descricao,
          ...
          (err2, result2) => {
            ...
            if (err2) {
              return res.status(500).json({
                erro: "Erro ao cadastrar empresário.",
